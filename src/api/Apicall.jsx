...
export const getSelectedJobDetails = async (params = {}, header = {}) => {
 ...
 export const registerTelementry = async (siteUrl, transactionId) => {
  ...
  telementryJson.events[0].edata.utm_source = localStorage.getItem('utm_source')? localStorage.getItem('utm_source'): '';


  try {
    const response = post(`${baseUrl}/jobs/telemetry`, telementryJson, {
      headers: {
        'Content-Type': 'application/json'
      }
    });
